    ...
    mongoClient.connect("mongodb://0.0.0.0:27017", async function (error, client) {
        ...
        app.post("/registration", async function (request, result) {
            ...
            if (email == global.adminEmail) {
                result.json({
                    status: "error",
                    message: "Rất tiếc, bạn không thể tạo tài khoản bằng địa chỉ email này."
                })
                return
            }
            ...
            bcryptjs.genSalt(10, function (error, salt) {
                bcryptjs.hash(password, salt, async function (error, hash) {
                    ...
                    result.json({
                        status: "success",
                        message: "Tài khoản đã được tạo. Vui lòng đăng nhập ngay bây giờ."
                    })
                })
            })
        })
        ...
        app.post("/cashOnDelivery", async function (request, result) {
            ...
            result.json({
                status: "success",
                message: "Đơn đặt hàng của bạn đã được nhận. Chúng tôi sẽ cho bạn biết qua E-mail về việc theo dõi đơn hàng của bạn.",
            })
        })
        ...
        app.post("/getConfigurations", async function (request, result) {
            ...
            if (configurations == null) {
                result.json({
                    status: "error",
                    message: "Vui lòng đặt cấu hình của bạn từ bảng điều khiển quản trị."
                })
                return
            }
