	...
	<div class="section no-pad-bot" id="index-banner" v-if="product != null">
		<div class="container-fluid">
			<div class="row">
				...
				<div class="col m6">
					<h1 v-text="product.name + ' (' + product.price + ' VNĐ)'"></h1>
					<p v-text="product.description"></p>
					<p>Số lượng trong kho: <span v-text="product.itemsInStock || 0"></span></p>

					<button v-if="isInCart" type="button" class="waves-effect waves-light btn"
						v-on:click="removeFromCart">Xóa khỏi giỏ hàng</button>
					<button v-else type="button" class="waves-effect waves-light btn" v-on:click="addToCart">Thêm vào giỏ
						hàng
					</button>
				</div>
			</div>
				...
				if (keyValue[0] == "products") {
					...
					for (let b = 0; b < products.length; b++) {
						if (products[b]._id == this._id) {
							...
							swal.fire("Xóa khỏi giỏ hàng", "Sản phẩm đã bị xóa khỏi giỏ hàng", "success")
							this.isInCart = false
