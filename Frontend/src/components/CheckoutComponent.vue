			...
			<div class="row">
				<div class="col m8">
					<div class="card">
						...
						<div class="card-body">
							<p v-if="cartEmpty" style="padding: 20px;">Giỏ hàng của bạn đang trống.</p>
								...
								<table class="table align-items-center justify-content-center mb-0" id="products-table">
									...
									<tbody>
										<tr v-for="(product, index) in products" v-bind:key="product._id">
											...
											<td v-text="product.price + ' VNĐ' + ' x ' + product.units"></td>
						...
						<div class="card-footer" v-if="!cartEmpty">
							<div class="row">
								<div class="col offset-m6 m6">
									<button type="button" class="waves-effect waves-light btn offset-m2"
										v-on:click="cashOnDelivery" style="margin-top: 20px;"
										v-bind:disabled="paymentLoading">Đăt hàng</button>
								</div>
							</div>
						</div>
					</div>
	...
	methods: {
		cashOnDelivery: function () {
			...
			swal.fire({
				...
				text: "Bạn chắc chắn muốn thanh toán khi nhận hàng.",
				icon: 'warning',
