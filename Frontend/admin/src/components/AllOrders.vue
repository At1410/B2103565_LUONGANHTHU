            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="font-weight-bolder">Tất cả đơn hàng</h4>
                    </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive p-0">
                            <table class="table align-items-center justify-content-center mb-0" id="table">
                                <thead>
                                    <tr>
                                        <th class="text-uppercase text-secondary opacity-7">Người dùng</th>
                                        <th class="text-uppercase text-secondary opacity-7">Địa chỉ</th>
                                        <th class="text-uppercase text-secondary opacity-7">Tổng</th>
                                        <th class="text-uppercase text-secondary opacity-7">Đơn hàng</th>
                                ...
                                <tbody>
                                    <tr v-for="(order) in orders" v-bind:key="order._id.toString()">
                                        ...
                                        <td>
                                            <span v-text="getTotal(order.cart, order.shippingCharges || 0)"></span> VNĐ
                                        </td>

                                        <td>
                                            <span v-if="order.paidVia == 'COD'" style="color: black;">Thanh toán khi nhận
                                                hàng</span>
                                        </td>
                                        ...
                                        <td>
                                            ...
                                            <form v-if="typeof order.status == 'undefined' || order.status == 'Xử lý'"
                                                v-on:submit.prevent="markAsCompleted">
                                                <input type="hidden" name="_id" v-bind:value="order._id" required />

                                                <input type="submit" name="submit" value="Đánh dấu hoàn thành"
                    ...
                    <div class="card-footer">
                        <button type="button" v-on:click="loadMore"
                            class="btn btn-lg bg-gradient-primary btn-lg w-100 mt-4 mb-0" ref="btnLoadMore">Xem
                            Thêm</button>
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        markAsCompleted: function () {
            ...
            swal.fire({
                title: 'Bạn chắc chắn?',
                text: "Bạn có muốn đánh dấu đơn đặt hàng này là đã hoàn thành?",
                icon: 'warning',
                    ...
                    if (response.data.status == "success") {
                        for (let a = 0; a < self.orders.length; a++) {
                            if (self.orders[a]._id == form._id.value) {
                                self.orders[a].status = "Hoàn thành"
                                break
                            }
                        }
